...
sampleIDIndices <- str_starts(colnames(sumTab), "Sample.Name")

for (s in sampleIDIndices) {
  if (s < tail(sampleIDIndices, n = 1)) {
    y <- colnames(sumTab)[s:s + as.numeric(params$sampleIDInsertFreq) - 1]
  } else {
    y <- colnames(sumTab)[s:length(colnames(sumTab))]
  }
  kable(x = select(sumTab, all_of(y)), 
        align = "l", 
        format = "latex")
}
```
