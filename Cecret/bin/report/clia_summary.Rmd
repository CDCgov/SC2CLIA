...
sampleIDColNames <- str_starts(colnames(sumTab), "Sample.Name")

for (s in sampleIDColNames) {
  if (s != tail(sampleIDColNames, n = 1)) {
    y <- colnames(sumTab)[which(str_detect(colnames(sumTab), s)) + params$sampleIDInsertFreq - 1]
  } else {
    y <- tail(colnames(sumTab), n = 1)
  }
  kable(x = select(sumTab, all_of(colnames(sumTab)[s:y])), 
        align = "l", 
        format = "latex")
}
```
