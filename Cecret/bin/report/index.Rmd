...
sumTab <- mutate(sumTab, sample = str_replace(sample, paste0("-", params$runID), "")) %>% 
          ...
          mutate(
            ...
            Total_Reads_Analyzed = cell_spec(Total_Reads_Analyzed,
                                             color = ifelse(Total_Reads_Analyzed < totMappedReads, "red", "black"), 
                              bold = ifelse(Total_Reads_Analyzed < totMappedReads, T, F)),
            Percent.Mapped.Reads = cell_spec(Percent.Mapped.Reads,
                                             color = ifelse(Percent.Mapped.Reads < perMappedReads, "red", "black"), 
                              bold = ifelse(Percent.Mapped.Reads < perMappedReads, T, F)),
            ...
            Coverage.S = cell_spec(Coverage.S, 
                                   color = ifelse(Coverage.S < sGeneCovThreshold, "red", "black"), 
                              bold = ifelse(Coverage.S < sGeneCovThreshold, T, F)),
            Percent.Ns.S = cell_spec(Percent.Ns.S,
                                     color = ifelse(Percent.Ns.S >= sPerNThreshold, "red", "black"), 
                              bold = ifelse(Percent.Ns.S >= sPerNThreshold, T, F)),
            vdr_sgene_orfshift = cell_spec(vdr_sgene_orfshift,
                                           color = ifelse(vdr_sgene_orfshift != sGeneFrameshiftThreshold, "red", "black"), 
                              bold = ifelse(vdr_sgene_orfshift != sGeneFrameshiftThreshold, T, F)))
