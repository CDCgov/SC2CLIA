...
# There's still a lot of clean up and optimization to be done on this file. Big loops need to be broken up, repeated blocks put into functions, etc. Note cache is turned on in block amp_cov; I don't think this will cause a problem when running multiple times in the same directory on different runs, but I don't know.
```
...
pbFiles <- list.files(path = paste(params$analysisDirFP, params$pacbamDir, sep = "/"),
                      full.names = TRUE,
                      recursive = TRUE)
  ...
  if (pool == 1) {
    for (f in 1:length(pbFilesPU_odd)) { # this gives the index of a file in the odd list
      tempPU <- read_tsv(pbFilesPU_odd[f]) %>%
                ...
                dplyr::mutate(SampleID = str_replace(SampleID, paste0("-", params$runID, params$pacbamFileSuffix), "")) %>%
