...
params:
  ...
  runID: "M347-21-010"
  pacbamDir: "pacbam"
  ampBED: "ref_docs/nCoV-2019.insert.bed"
  ampMinCov: 30
  ampMeanCov: 100
  ampMaxNFail: 5
  ampMinNPass: 93
...
# Unlike most other params set in this report template, covFileDir and covFileSuffix are not controlled by options in the config.R script. It is assumed that the Cecret run directory structure is maintained and the report script will look for the directories and files named in params in one directory up from the run directory. To change these settings, modify params.
```
  ...
  if (pool == 1) {
    for (f in 1:length(pbFilesPU_odd)) { # this gives the index of a file in the odd list
      tempPU <- read_tsv(pbFilesPU_odd[f]) %>%
                ...
                dplyr::mutate(SampleID = str_replace(SampleID, paste0("-", params$runID, ".primertrim.sorted.pileup"), "")) %>%
