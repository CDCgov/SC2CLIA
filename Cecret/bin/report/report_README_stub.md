...
| QC | coverage_after_trimming | Coverage.Post.Trim | Percent Genome Coverage | >= 90% with SME discretion to got as low as 60% if mutations reported | |
...
| used in QC calc | Total_Reads_Analyzed | Total.Reads.Analyzed | Mapped Reads | 100,000 | |
| QC | %_N | %N | not included | <10% | |
| not reported | ave_cov_depth | Mean.Cov.Depth | not included | | |
| not reported | %_Reads_Matching_SC2_Ref | %Reads.Mapping.SC2 | not included | |
...
| reported | S_aa_indels | AA.Changes.S | Spike Protein Substitutions | not null | list of insertions, deletions, and substitions found in the amino acids reported for the S gene |
...
| QC | Percent.Mapped.Reads | Percent.Mapped.Reads | Percent Mapped Reads | >=65% | Calculated as (Total Reads Analyzed / Total Reads)*100 |
